...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            ...
            <form action="{{ route('admin.pratice-dialogue.update', $practiceDialogue->id) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" value="{{ $practiceDialogue->title }}" name="title" class="form-control"
                        required>
                </div>
                ...
                <div id="segment-wrapper">
                    @foreach ($practiceSegments as $loopIndex => $practiceSegment)
                        <div class="segment-block mb-4 mt-4 border p-6 rounded-2xl bg-light">
                            ...
                            @endif --}}
                                <input type="file" name="segments[{{ $loopIndex }}][segment_path]"
                                    class="form-control mt-3" accept=".mp3,.wav">
                            </div>

                            <div class="mb-1 p-2">
                                <div id="sample-waveform-{{ $practiceSegment->id }}" style="width: 100%; height: 80px;">
                                </div>
                                <button type="button" class="btn btn-success play-btn"
                                    data-id="sample-{{ $practiceSegment->id }}">▶ Play</button>
                            ...
                            @endif --}}
                                <input type="file" name="segments[{{ $loopIndex }}][sample_response]"
                                    class="form-control mt-3" accept=".mp3,.wav">
                            </div>
                            ...
                            <div class="row p-2">
                                <div class="mb-1 p-2 col-sm-6">
                                    ...
                                    <input type="text" name="segments[{{ $loopIndex }}][answer_eng]"
                                        value="{{ $practiceSegment->answer_eng }}" class="form-control" required>
                                </div>
                                <div class="mb-1 p-2 col-sm-6">
                                    <label class="form-label answer-languages-label">Answer Second Language</label>
                                    <input type="text" name="segments[{{ $loopIndex }}][answer_second_language]"
                                        value="{{ $practiceSegment->answer_other_language }}" class="form-control"
                                        required>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        @foreach ($practiceSegments as $practiceSegment)
            ...
            wavesurfers['segment-{{ $practiceSegment->id }}'].load(
                "{{ asset('storage/' . $practiceSegment->segment_path) }}");
            ...
            @if ($practiceSegment->sample_response)
                ...
                wavesurfers['sample-{{ $practiceSegment->id }}'].load(
                    "{{ asset('storage/' . $practiceSegment->sample_response) }}");
            @endif
        @endforeach
        ...
        <div class="removebtn d-flex justify-content-between">
            ...
            <button type="button" class="btn btn-danger remove-segment float-end">✖
                </button>
</div>
            <div class="mb-3 p-2">
