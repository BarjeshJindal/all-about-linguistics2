...
@section('content')
                    ...
                    @foreach ($labels as $label)
                        <li>
                            <button type="submit" class="dropdown-item d-flex  align-items-center select-label"
                                data-practice-id="{{ $practice->id }}" data-label-id="{{ $label->id }}"
    ...
    <div class="dialogue-container row p-2">
        <div class="diloce-title col-lg-9 bg-camera">
            <h2>Dialogue - {{ $practice->title }}</h2>
            <p class="desc">{{ $practice->description }}</p>
            <input type="hidden" id="dialogueIdInput" value="{{ $practice->id }}">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <ul>
                                <li>
                                    Click <button class="btn btn-sm btn-outline-primary">▶️ Play / Pause</button> to start
                                    the
                                    dialogue
                                </li>
                            </ul>
                            ...
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                ...
                                <div class="bg-[#818a91] mt-4">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-2">
                                        <div>
                                            ...
                                            <input type="range" min="0" max="1" step="0.01"
                                                value="1" id="volume-{{ $segment->id }}" style="width: 80px;" />
                                    ...
                                    data-segment-number="{{ $loop->iteration }}">
                                        ...
                                        {{-- notes --}}
                                        <button type="button" class="btn btn-light-success" data-bs-toggle="modal" data-bs-target="#noteModal">
                                        📝 Take Note
                                        </button>
                                        {{-- notes --}}
...
<div class="mb-4">
        <button onclick="submitAllResponses()" class="btn btn-success mt-4">Submit Test </button>
        </div>
         <!-- Notes Modal -->
    <div class="modal fade" id="noteModal" tabindex="-1" aria-labelledby="noteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="noteModalLabel">Your Notes</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form id="noteForm" >
                @csrf
                <input type="hidden" name="dialogue_id" value="{{ $practice->id }}">
                <input type="hidden" name="type_id" value="1"> {{-- 2 = VIP Exam --}}
                
                <div class="mb-3">
                    <label for="note" class="form-label">Note</label>
                    <textarea class="form-control" name="note" id="noteTextarea" rows="6">{{ $note?->note ?? '' }}</textarea>
                </div>
                </form>
            </div>
            <div class="modal-footer">
                ...
                document.querySelectorAll(".segment-container").forEach(container => {
                    ...
                    startBtn.onclick = async () => {
                        ...
                        waveform.once('finish', async () => {
                            startBtn.innerText =
                            'Record audio after beep...'; // Indicate beep phase
                            stopBtn.disabled = true; // Disable stop until beep finishes

                            const beepSound = new Audio(
                            '/sounds/beep_2sec.mp3'); // Path to your file
                            beepSound.play();

                            beepSound.onended = async () => {
                                // ✅ After beep ends, start recording
                                startBtn.innerText =
                                'Recording...'; // Now it's actually recording
                    ...
                    stopBtn.onclick = () => {
                       ...
                       // ✅ enable Next button only after stop
        ...
        {{-- notes --}}
         <script>
            document.getElementById('saveNoteBtn').addEventListener('click', function () {
                ...
                // ✅ Add CSRF token from the hidden input (_token field generated by @csrf)
            ...
            // Get current dialogue ID from hidden input
            const dialogueId = document.getElementById('dialogueIdInput').value;

            // type_id is 1 fixed for pratice dialogue
            const typeId = 1;

            // Fetch the existing note
            fetch(`naati/notes/${dialogueId}/${typeId}`)
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
                ...
                .catch(err => console.error("Fetch note error:", err));
        });
